...
export async function filterByRating() {
    ...
    const firstRate = firstRating.querySelector("input[type='radio']:checked");

    // secondRate is the value of the second rating
    const secondRating = document.querySelector(".filter__second-rate");
    const secondRate = secondRating.querySelector("input[type='radio']:checked");
    ...
    if (firstRate && secondRate) {
        ...
        allChallenges.forEach((challenge) => {
            ...
            i++;
            if (challengeRating >= firstRate.value && challengeRating <= secondRate.value) {
